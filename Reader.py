...
def extract_trade_info_transaction_statement(pdf_text: str):
    ...
    if start_idx != -1:
        ...
        for i in range(start_idx + 1, min(len(lines), start_idx + 10)):
            ...
            bad_words = [
                "Execution Venue", "Market Value", "Amount", "Order", "Account", 
                "Baader", "Bank", "UniCredit", "München", "Munich", "Tax", "WKN:", "ISIN:",
                "Client", "Portfolio", "Reference"
            ]
            if any(bw.lower() in line.lower() for bw in bad_words):
                continue
            
            # Zahlen/Datum ignorieren
            if re.match(r'^[\d\.,\s:-]+$', line): continue # Nur Zahlen/Datum
            if re.search(r'\d{4}-\d{2}-\d{2}', line): continue # Zeile enthält Datum
    ...
    if not asset_name:
        ...
        if q_idx != -1:
            ...
            for line in snippet.splitlines():
                line = line.strip()
                if "Execution Venue" in line: continue # WICHTIG
...
def extract_trade_info_contract_note(pdf_text: str):
    ...
    # Typ & Asset
    head_match = re.search(r'^(Buy|Sell)\s+([\d\.,]+)\s*(?:pc\.|Stk\.)?\s+(.+)$', pdf_text, re.MULTILINE)
    ...
    for trade in all_trades:
        ...
        if trade["Trade Type"] == "Buy":
            ...
            if row:
                ...
                ws[f"G{row}"] = qty
                ws[f"H{row}"] = price
                ws[f"I{row}"] = (qty * price) - fee
                ws[f"J{row}"] = fee
                ws[f"K{row}"] = f"=F{row}-B{row}"

                ws[f"N{row}"] = f"=I{row}-E{row}"
                ws[f"M{row}"] = f"=N{row}/E{row}"
                ...
                if col_profit_day_net and col_profit_abs_net:
                    ws[f"{col_profit_day_net}{row}"] = f"=IF(K{row}>0, {col_profit_abs_net}{row}/K{row}, {col_profit_abs_net}{row})"
